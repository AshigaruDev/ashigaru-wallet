...
public class Whirlpool {
    ...
    public static final String PARTNER_ID_ASHIGARU = "ashigaruterminal";

    public static final List<Network> WHIRLPOOL_NETWORKS = List.of(Network.MAINNET, Network.TESTNET, Network.TESTNET4);
    ...
    private WhirlpoolWalletConfig computeWhirlpoolWalletConfig(HostAndPort torProxy) {
        ...
        whirlpoolWalletConfig.setPartner(PARTNER_ID_ASHIGARU);
    ...
    public Pool getPool(String poolId) {
        try {
            return getPools(null, false).stream().filter(pool -> pool.getPoolId().equals(poolId)).findFirst().orElse(null);
        } catch(Exception e) {
    ...
    public Collection<Pool> getPools(Long totalUtxoValue, boolean refresh) throws Exception {
        if (refresh) {
            this.poolSupplier.refresh();
        } else {
    ...
    public static class PoolsService extends Service<Collection<Pool>> {
        ...
        public PoolsService(Whirlpool whirlpool, Long totalUtxoValue, boolean refresh) {
        ...
        protected Task<Collection<Pool>> createTask() {
            return new Task<>() {
                protected Collection<Pool> call() throws Exception {
                    return whirlpool.getPools(totalUtxoValue, refresh);
                }
            };
        }
    }
